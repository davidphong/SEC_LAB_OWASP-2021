...
⚠️ Lỗ hổng Path Traversal trong chức năng hiển thị ảnh cho bài viết blog.
   Kẻ tấn công có thể truy cập các file nhạy cảm trên máy chủ.
"""

from flask import (Flask, render_template, request, redirect, url_for,
                   session, g, abort, jsonify, flash, send_file)
...
app.config.update(
    ...
    IMAGE_FOLDER=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static')
)

# Đảm bảo thư mục uploads tồn tại
os.makedirs(os.path.join(app.config['IMAGE_FOLDER'], 'uploads'), exist_ok=True)
...
# ---------------- Endpoint lấy ảnh cho blog (có lỗ hổng path traversal) ----------------
...
def admin():
    ...
    if g.user.role != "admin":  # Kiểm tra trực tiếp từ dữ liệu của user, không dùng session
        ...
        abort(403)
    flag = "VNPT{P4th_Tr4v3rs4l_Vuln3r4b1l1ty_F0und_1n_Im4g3_L04d1ng}"
